...
$$A,B,\pi$$ 称为隐马尔可夫模型的三要素。如果加上一个具体的状态集合$$Q$$和观测序列$$V$$，则构成了HMM的五元组，又是一个很有中国特色的名字吧。
...
分别对应着 $$A$$，$$B$$ 和 $$\pi$$。虽然该 class 目前什么都不能做，但是很快我们就能让它派上实际用场。
...
2. 学习问题。已知观测序列 $$O=(o_1,o_2,\cdots,o_T)$$，估计模型 $$\lambda=(A,b,\pi)$$ 参数，使得在该模型下观测序列概率 $$P(O\vert\lambda)$$ 最大。即用极大似然估计得方法估计参数。
3. 预测问题，也称为解码问题。已知模型 $$\lambda=(A,b,\pi)$$ 和观测序列 $$O=(o_1,o_2,\cdots,o_T)$$，求对给定观测序列条件概率 $$P(I\vert O)$$ 最大的状态序列 $$I=(i_1,i_2,\cdots,i_T)$$。即给定观测序列，求最有可能的对应的状态序列。
...
3. 终止 $$P(O\vert\lambda)=\sum_{i=1}^N \alpha_T(i)$$ 由于到了时间T，一共有N种状态发射了最后那个观测，所以最终的结果要将这些概率加起来。
...
定义（后向概率）：给定隐马尔可夫模型 $$\lambda$$，定义在时刻 $$t$$ 状态为 $$q_i$$ 的条件下，从 $$t+1$$ 到 $$T$$ 的部分观测序列为 $$o_{t+1},o_{t+2},\cdots,o_T$$ 的概率为后向概率。记作 $$\beta_t(i)=P(o_{t+1},o_{t+2},\cdots,o_T\vert i_t=q_i, \lambda)$$
...
3. $$P(O\vert\lambda)=\sum_{i=1}^N \pi_ib_i(o1)\beta_1(i)$$ 最后的求和是因为，在第一时间点上有 $N$ 种后向概率都能输出从2到T的观测序列，所以乘上输出 $O_1$ 的概率后求和得到最终结果。
...
假设给定训练数据只包含S个长度为T的观测序列 $$\{O_1,O_2,\cdots,O_S\}$$ 而没有对应的状态序列，目标是学习隐马尔可夫模型 $$\lambda=(A,B,\pi)$$ 的参数。我们将观测序列数据看作观测数据O，状态序列数据看作不可观测的隐数据I，那么隐马尔可夫模型事实上是一个含有隐变量的概率模型 
...
1. 确定完全数据的对数似然函数
2. EM算法的E步：求Q函数 $$Q(\lambda,\overline{\lambda})$$ $$Q(\lambda,\overline{\lambda})=\sum_I \log P(O,I\vert\lambda)P(O,I\vert\overline{\lambda})$$ 其中，$$\overline{\lambda}$$ 是隐马尔可夫模型参数的当前估计值，$$\lambda$$ 是要极大化的隐马尔可夫模型参数。（Q函数的标准定义是：$$Q(\lambda,\overline{\lambda})=E_I[\log P(O,I\vert\lambda)\vert O,\overline{\lambda}]$$，式子内部其实是条件概率，其中的 $$O, \overline{\lambda}$ 对应 $\frac{1}{P(O\vert\overline{\lambda})}$$；其与 $$\lambda$$ 无关，所以省略掉了。） $$P(O,I\vert\lambda)=\pi_{i_1}b_{i_1}(o_1)a_{i_1i_2}b_{i_2}(o_2)\cdots a_{i_{T-1}i_T}b_{i_T}(o_T)$$
这个式子从左到右依次是初始概率、发射概率、转移概率、发射概率……
于是函数$$Q(\lambda,\overline{\lambda})$$可以写成：
$$Q(\lambda,\overline{\lambda})=\sum_I \log \pi_{i_1}P(O,I\vert\overline{\lambda}) $$
$$+ \sum_I \left(\sum_{t=1}^{T-1} \log a_{i_t i_{t+1}} \right) P(O,I\vert\overline{\lambda}) $$
$$+ \sum_I \left(\sum_{t=1}^T \log b_{i_t}(o_t) \right) P(O,I\vert\overline{\lambda})$$
式中求和都是对所有训练数据的序列总长度T进行的。这个式子是将 $$P(O,I\vert\lambda)=\pi_{i_1}b_{i_1}(o_1)a_{i_1i_2}b_{i_2}(o_2)\cdots a_{i_{T-1}i_T}b_{i_T}(o_T)$$ 代入 $$Q(\lambda,\overline{\lambda})=\sum_I \log P(O,I\vert\lambda)P(O,I\vert\overline{\lambda})$$ 后，将初始概率、转移概率、发射概率这三部分乘积的对数拆分为对数之和，所以有三项。
3. EM算法的M步:极大化Q函数$$Q(\lambda,\overline{\lambda})$求模型参数$A,B,\pi$$，由于要极大化的参数在Q函数表达式中单独地出现在3个项中，所以只需对各项分别极大化。
**第1项可以写成：**
$$\sum_I \log \pi_{i_0}P(O,I\vert\overline{\lambda})=\sum_{i=1}^N \log \pi_i P(O,i_1=i\vert\overline{\lambda})$$
注意到$\pi_i$满足约束条件利用拉格朗日乘子法，写出拉格朗日函数：
$$\sum_{i=1}^N \log \pi_i P(O,i_1=i\vert\overline{\lambda}) + \gamma \left(\sum_{i=1}^N \pi_i - 1 \right)$$
对其求偏导数并令结果为0
$$\frac{\partial}{\partial\pi_i} \left[\sum_{i=1}^N \log \pi_i P(O,i_1=i\vert\overline{\lambda}) + \gamma \left(\sum_{i=1}^N \pi_i - 1 \right) \right] = 0$$
得到
$$P(O,i_1=i\vert\overline{\lambda})+\gamma\pi_i=0$$
这个求导是很简单的，求和项中非i的项对πi求导都是0，logπ的导数是1/π，γ那边求导就剩下πi自己对自己求导，也就是γ。等式两边同时乘以πi就得到了上式。
对i求和得到γ：
$$\gamma=-P(O\vert\overline{\lambda})$$
代入 $$P(O,i_1=i\vert\overline{\lambda})+\gamma\pi_i=0$$ 中得到：
$$\pi_i=\frac{P(O,i_1=i\vert\overline{\lambda})}{P(O\vert\overline{\lambda})}$$
**第2项可以写成：**
$$\sum_I \left(\sum_{t=1}^{T-1} \log a{i_t i_{t+1}} \right) P(O,I\vert\overline{\lambda}) = \sum_{i=1}^N \sum_{j=1}^N \sum_{t=1}^{T-1} \log a_{ij} P(O,i_t=i,i_{t+1}=j\vert\overline{\lambda})$$
类似第1项，应用具有约束条件 $$\sum_{j=1}^N a_{ij}=1$$ 的拉格朗日乘子法可以求出
$$a_{ij}=\frac{\sum_{t=1}^{T-1}P(O,i_t=i,i_{t+1}=j\vert\overline{\lambda})}{\sum_{t=1}^{T-1}P(O,i_t=i|\overline{\lambda})}$$
**第3项为：**
